  ...
  const loadStats = async () => {
    ...
    dates.forEach((date, index) => {
      ...
      if (viewMode === 'week') {
        ...
        chartLabels.push(['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d.getDay()]);
      } else if (index % 5 === 0) {
        const d = new Date(date);
    ...
    if (validData.length > 0) {
      setAverage(Math.round(validData.reduce((a, b) => a + b, 0) / validData.length));
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.statCard}>
          <Text style={styles.statValue}>{Math.floor(total / goal)}</Text>
